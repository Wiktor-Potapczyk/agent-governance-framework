...
> **Adopted (2026-06-11) - procedure-as-workflows, partial.** Two process skills (process-planning, process-pentest) now run as **deterministic workflow scripts** that make their dispatch sequence happen *by construction* - routing-as-code, not execution-as-code (the script encodes which agents run, in what order, with what typed handoffs and gates; agents still reason freely inside each step). Their SKILL.md files are thin invokers; the prose survives as spec-of-record and fallback, and the machine-readable dispatch contract survives as a read-only verification source. The remaining process skills are still prose-plus-hooks; expansion is gated on evidence from real enforced runs (success metric = output quality, since dispatch-by-construction makes "did we dispatch?" tautological). The owner waived the original pre-adoption calibration baseline. Rationale: see ADR-0004; full write-up and the engine-proof finding live in the research repo (`procedure-layer-as-workflows`).
