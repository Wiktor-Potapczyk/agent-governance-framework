...
- **`hooks/disabled/em-dash-guard.py`'s own dash table was broken by an earlier prose scrub.** The workspace-to-repo porting pass that replaces the maintainer's name with "the owner" also runs a dash-to-punctuation scrub over prose, and that scrub is not scoped to prose: it had rewritten five of the nine literal Unicode glyphs in `BLOCKED_DASHES` (figure dash, en dash, em dash, horizontal bar, minus sign) into plain colons. Since the hook's own detector checks `if glyph in prose`, an armed copy of this hook would have matched on any ordinary colon and reported five fabricated dash violations on virtually every response. The two-em and three-em dash entries were untouched (their glyphs are not adjacent to the corrupted run), which is why this was not caught by inspection. Restored from the workspace source; verified empirically (a plain colon now returns zero matches, a literal em dash and en dash are each correctly detected).

### Held items resolved

- **`config-protection.py` deregistered and moved to `hooks/disabled/`**, closing the item this repo's own 2026-08-19 entry flagged as "held back deliberately... until the corresponding restructure decision... is made deliberately rather than as a merge side effect." The workspace retired this guard on 2026-08-07 after it hard-blocked an already-approved batch of edits twice in one day; `hooks/disabled/README.md` now carries the reason. `agent-registry-check.py` was retired from the workspace in the same pass and is deregistered and moved alongside it, though no independent failure narrative was recorded for it specifically -- documented as an honest gap rather than an invented one.
- **`epistemic-check.py`'s active, registered top-level copy removed.** It was shipping simultaneously as registered-and-live (`settings/settings.json.template`) and as documented-disabled-after-failure (`hooks/disabled/README.md`: "never blocked in practice... rubber-stamped everything"), which is a direct contradiction an adopter deciding whether to arm it would have hit. The single surviving copy is the already-documented one in `hooks/disabled/`.
- **`weekly-usage.py` consolidated from two duplicate copies into one archived copy.** `docs/reference/hooks.md` had flagged the `hooks/` + `hooks/disabled/` duplication as a known "state flagged for consolidation." It is retired from the workspace's active toolset; the single surviving copy now lives at `_archived/hooks/weekly-usage.py` with a header explaining what happened, alongside `context-fill-log.py` (a genuine no-op stub, unlike this one, which is a working CLI utility -- the architecture.md component-count table now distinguishes the two).
- **The unicode-hygiene hook's non-test half is no longer held.** The 2026-08-19 entry noted "a unicode-hygiene hook trio exists upstream but is held: its tests depend on glyph-level fixtures and workspace paths that need a fixture-shipping strategy here first." `unicode-hygiene-check.py` and its shared library `_unicode_hygiene.py` carry no such dependency (pure stdlib, no fixtures) and now ship, unregistered/opt-in, in `hooks/`. `test_unicode_hygiene.py` remains held: it needs twelve committed glyph-level fixture files under a path this repo's `.gitignore` currently excludes wholesale (`hooks/_test_fixtures/`), and a prior entry already flagged the same gitignore-vs-tracked-fixture tension for a different suite. That is a decision for the maintainer, not a default this pass should invent.

### Not ported (workspace-specific or unsafe to publish)

Five hooks live in the workspace's own `.claude/hooks/` but are not published here, evaluated on content rather than name:

- `proactivity-check.py` -- hardcodes the maintainer's own Windows filesystem path and matches literally on the maintainer's first name as a task-ownership marker throughout; not adaptable without a rewrite of its actual logic, not just a path substitution.
- `repo-sync-cadence-trigger.py` -- self-referential to this exact two-repo publishing arrangement (hardcoded paths to this repo and its research-repo sibling, named skill references); the pattern of "remind me when a downstream repo goes stale" is generalizable but this specific file is not a drop-in.
- `vault-structure-check.py` -- **excluded on both grounds at once.** Vault-specific (enforces a personal Obsidian frontmatter/tag/wiki-link convention meaningful only to one person's note-taking setup) AND it hardcodes the employer's internal name and other project codenames as literal canonical-tag strings, which this repo's NDA constraint forbids publishing under any circumstance. Not scrubbed, not adapted -- left out entirely per that constraint.
- `sdlc-evidence-verifier.py` -- **excluded on safety grounds.** Hardcodes a real production server IP address, a real public hostname, and a root SSH target for the maintainer's own infrastructure. Not scrubbed, not adapted -- left out entirely; a file this specific to one person's live infrastructure has no publishable form.
- (`em-dash-guard.py` and `prose-codes-check.py` were already present in `hooks/disabled/` from an earlier pass and needed documentation, not porting; see Fixed above and `hooks/disabled/README.md`.)

### Docs

- **Every count this pass changed was re-derived, not incremented by hand**, and checked with `skills/core/doc-consistency/check_doc_consistency.py .doc-consistency.json`: 0 mismatches across 13 checks. Active enforcement hooks 45 -> 42; production hook files 55 -> 53; shared hook libraries 10 -> 11 (`_unicode_hygiene.py` added); hook test files 53 -> 52 (`test_config_protection.py` moved out of the counted directory alongside its hook); opt-in/disabled hook files 9 -> 11. `settings/settings.json.template.README.md`'s per-event hook table (not manifest-tracked) was reconciled by hand for the same reason. `INSTALL.md` carried two pre-existing stale figures unrelated to this pass's own edits (a "6 hooks" minimal-set claim against an actual 12, and a "35 hooks" full-set claim last correct before several earlier passes) sitting in the same sentences this pass had to touch anyway; corrected alongside rather than left contradicting the number next to them.
- Internal Markdown link scan and JSON-template validation (the other two `.github/workflows/docs.yml` steps) both re-run clean. The hook suite (`pytest hooks/ -q`) passes at 1134 passed / 11 skipped / 0 failed after all moves and removals; PyYAML is present in this environment so both CI legs exercise the same code path here, and nothing this pass touched is on the PyYAML-guarded branch.
