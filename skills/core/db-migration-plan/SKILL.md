...
**Database-indexing learning goal (Wiktor):** whenever this skill proposes an index, it MUST narrate the choice — which columns, which index type (btree / hash / GIN / BRIN / partial / composite), the column order and why, and the read-vs-write tradeoff. An index added without a stated reason is a plan defect.
...
- This skill plans; it does not run DDL. Applying the migration stays a human/n8n/Supabase action — surface the plan, let Wiktor apply.
