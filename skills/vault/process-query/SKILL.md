...
- If a relevant raw source exists in `Inbox/`, `Clippings/`, or a project's `work/` (search `Projects/*/work/` AND `Projects/*/*/work/`: projects nest one level, so `Projects/Personal/Finance/work/` is invisible to a single-star glob), suggest running `process-ingest` on it first
...
status: active
wiki_status: bootstrap   # filed-back query pages always start bootstrap; the owner promotes to ratified
source:
...
- **Filed-back pages are always `wiki_status: bootstrap`.** The skill never self-ratifies. the owner promotes to `ratified`.
