...
- **Bootstrap mode is active until 10 ratified entries exist.** Until then, every wiki page you create gets `wiki_status: bootstrap` frontmatter and is surfaced for the owner's review before treating it as ground truth.
...
status: "#active"
wiki_status: bootstrap   # all v1 pages start bootstrap; the owner promotes to ratified
source:
...
Append to `Vault/log.md` (vault root) using the schema per CLAUDE.md `## Karpathy LLM-Wiki Architecture` section:
...
- **All new wiki pages start `wiki_status: bootstrap`.** Promoted to `ratified` only by the owner review (M3 defer-wiki gate).
