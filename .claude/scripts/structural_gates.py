...
def check_c5_dispatch_name_resolution() -> dict:
    ...
    # Agent types built into Claude Code resolve to the host, not to a registry
    # entry (2026-09-21, harness audit 1: the loader adds them so a truthful
    # MUST DISPATCH naming Explore or general-purpose no longer reads as empty).
    builtin = set()
    try:
        hooks_dir = str(Path(__file__).resolve().parent.parent / "hooks")
        if hooks_dir not in sys.path:
            sys.path.insert(0, hooks_dir)
        from _known_dispatch_names_loader import BUILTIN_AGENT_TYPES  # type: ignore
        builtin = set(BUILTIN_AGENT_TYPES)
    except Exception:
        builtin = {"explore", "general-purpose", "plan", "web-fetch", "fork", "claude"}
