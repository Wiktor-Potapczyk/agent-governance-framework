...
def generate(inventory_path, rationale_path, out_dir, kinds=None, dry_run=False):
    ...
    if not dry_run:
        # D9 exempts this one file on purpose: page content stays wall-clock
        # free, but the stamp's whole job is recording when this run happened.
        stamp_data = {
        ...
        stamp_text = json.dumps(stamp_data, ensure_ascii=False, indent=2) + "\n"
        with open(out / "_generated.json", "wb") as fh:
            fh.write(stamp_text.encode("utf-8"))
    return report
