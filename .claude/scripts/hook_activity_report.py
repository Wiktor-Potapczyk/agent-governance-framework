...
# .claude/scripts/<this file> -> .claude/scripts -> .claude -> vault root.
# The env override stays as the override; the self-located path is the
# default. It used to be the other way round: the default named one laptop,
# so every caller that did not set VAULT_DIR (SessionStart, /process-lint,
# the owner running this by hand) read a nonexistent root on any other
# machine, and the runner worked around it by setting VAULT_DIR itself.
_SELF_LOCATED_VAULT = Path(__file__).resolve().parents[2]
VAULT = Path(os.environ.get("VAULT_DIR") or _SELF_LOCATED_VAULT)
...
def _writes_any_log(hook_name: str) -> bool:
    for sub in ("hooks", "scripts"):
        ...
        if p.exists():
            ...
            return _includes_any(t, _LOG_SIG)
    return False
...
def _includes_any(text, needles):
    """True iff at least one needle appears in text (existing matrix semantics)."""
    return any(n in text for n in needles)


def _includes_all(text, needles):
    """True iff every needle appears in text. ECC-adopted primitive
    (observability-readiness.js 102-104; ECC-LEARN-H1, owner GO 2026-09-06).
    Not yet wired to a signature class: flipping a class from any- to
    all-semantics changes matrix behavior and needs its own ruling."""
...
def matrix_rows():
    ...
    for name in sorted(events):
        ...
        classes = []
        sinks = []
        if _includes_any(src, _A_SIG):
            classes.append("A")
            sinks.append("governance-log")
        if _includes_any(src, _B_SIG):
            classes.append("B")
...
def _is_floor(hook_name: str) -> bool:
    ...
    return _includes_any(src, _FLOOR_SIG)


def _writes_govlog(hook_name: str) -> bool:
    src = _hook_source(str(hook_name)) or ""
    return _includes_any(src, _A_SIG)
