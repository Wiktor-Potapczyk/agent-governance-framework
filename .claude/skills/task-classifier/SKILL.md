...
- **`MUST DISPATCH: none` on non-Quick is a hook-blocker**: `none` is valid only for Quick; non-Quick must list at minimum `process-qa`. (`pm` was also required until the mandate was suspended 2026-09-09.) The Stop hook verifies and blocks the response if a named dispatch is missing or unrun.
...
| ~~**ALL non-Quick**~~ | ~~**PM**~~ | ~~**pm**~~ | ~~Every non-Quick task gets PM oversight.~~ **SUSPENDED 2026-09-09 by owner ruling. PM is no longer a floor rule and `pm` is no longer required in MUST DISPATCH. Invoke it deliberately when project state genuinely needs a checkpoint. See CLAUDE.md Task Classification for the ruling and the reinstatement switch.** |
...
**PM enforcement: SUSPENDED 2026-09-09 (owner ruling).** `pm` is no longer required in MUST DISPATCH and PM no longer runs on every non-Quick task. Runtime switch: `PM_MANDATE_SUSPENDED` in `.claude/hooks/classifier-field-check.py`. One rule survives the suspension: if you DO name `pm` in MUST DISPATCH, the dispatch-compliance hook still requires a real pm-orchestrator dispatch, and an inline PM CHECKPOINT REPORT does not satisfy it (CLAUDE.md F-2).

**PM reactive triggers (now advisory, not mandatory):** these signals are the strongest remaining reasons to invoke PM deliberately. They no longer force `pm` into MUST DISPATCH, and they no longer escalate Quick to Analysis on their own:
...
- **PM checkpoint (`pm`) is NO LONGER required**: mandate suspended 2026-09-09 by owner ruling. Add `pm` only when you judge a checkpoint is genuinely warranted, and dispatch it for real if you name it.
...
5. **After pentest completes:** consider invoking `/pm` for a checkpoint on project state and next action. Optional since the PM mandate was suspended 2026-09-09; a completed pentest is a reasonable occasion to choose it.
...
## O16 ARS routing appendix (moved from CLAUDE.md, 2026-09-02)

Moved verbatim from CLAUDE.md ## CRITICAL RULE: Task Classification (O16 trim, ruling
row S13 of [[2026-09-02-o16-row-rulings-merged]]). CLAUDE.md keeps the one-line routing
rule (academic output routes to ARS commands); this appendix is the full trigger-phrase
table.

**Academic/literature research path (added 2026-05-13, ARS plugin v3.7.x installed):** use `/ars-full` (research → write → review → revise → finalize), `/ars-plan` (Socratic planning), `/ars-lit-review` (annotated bibliography), `/ars-revision-coach` (reviewer-comment parsing), `/ars-citation-check` (citation error report), `/ars-disclosure` (venue-specific AI-usage statement), `/ars-format-convert` (LaTeX/DOCX/PDF/Markdown), `/ars-abstract` (bilingual abstract + keywords), `/ars-outline`, `/ars-revision`: academic-specialist with PRISMA, APA 7.0, meta-analysis, peer-review simulation. **Decision axis: expected output type.** Academic document (paper, systematic review, annotated bibliography, peer-review response) → ARS commands. Business deliverable / general research / report → `process-research`. Marketing or audience-facing content → `process-build` (DOMAIN: content), NOT ARS. Trigger phrases that flag ARS: "write a paper", "academic paper", "literature review", "annotated bibliography", "PRISMA", "systematic review", "meta-analysis", "APA", "peer review", "respond to reviewers", "journal submission", "abstract for a paper". Generic "research X for me" → `process-research`. Ambiguous triggers ("deep research", "help me think through", "literature review" in non-academic context) → ask before invoking. "Write a paper" defaults to ARS only when academic context is present; otherwise treat as content task.
