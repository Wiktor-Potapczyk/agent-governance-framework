...
- A task where a self-graded loop (e.g. a Ralph loop with a structural completion promise) would declare "done" on shallow material
...
This is enforced: `.claude/hooks/verifier-gate-check.py` (a Stop hook) blocks completion when this skill was invoked but no separate-verifier `Agent` dispatch is found in the transcript. The verifier dispatch's `description` MUST contain the word "verifier" so the hook can identify it.
...
- Frontmatter + at least one inbound `[[wikilink]]` per vault stewardship rules.
...
- Reference implementation: the SOTA-RECON-2 depth pass — `Projects/Agent-Governance-Research/work/2026-05-19-sota-recon-depth-backlog.md` is a real ledger this harness produced (23 units, 5 workers, 1 separate verifier, 22 VERIFIED / 1 UNREACHABLE-VALID / 0 FAIL).
- This skill is the structured form of the CLAUDE.md ralph-loop doctrine ("loop body delegates to fresh sub-agents"). A Ralph loop can drive this harness, but the loop only orchestrates the ledger — it never investigates inline.
