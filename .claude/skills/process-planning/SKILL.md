...
**H10 fix (2026-04-18): route through `process-research`, not downstream researchers directly.**

Invoke the `process-research` skill via the Skill tool and pass the research question. `process-research` owns: scope definition (RESEARCH SCOPE block), path selection (direct dispatch vs Ralph Loop), synthesis mandate when 2+ agents contribute, and final report-generator pass. Bypassing it and dispatching `technical-researcher` / `research-analyst` directly skips these gates — the audit flagged this as a silent quality bypass.

The correct invocation pattern from inside process-planning:
- Decide the research question needed for the plan.
- Invoke `process-research` (Skill tool) with a self-contained prompt stating the question, known facts, and the plan-context it will feed into.
- Wait for process-research to return its synthesized findings + report path.
- Use those findings as input to Step 3 (implementation-plan).

If research is not needed (requirements are clear, domain is familiar), skip to Step 3.

For a single-fact lookup (one grep / one file read / one URL fetch with no synthesis) you may handle it inline — but the moment synthesis, comparison, or multi-source gathering is involved, route through `process-research`. Source of truth and falsification hygiene depend on the entry point not being skipped.
...
For MCP-domain planning (MCP servers/clients; GAP-5 binding 2026-07-10): **you MUST dispatch `mcp-server-architect`** alongside implementation-plan for the protocol/architecture design, with **`mcp-developer`** named in the plan as the bound downstream implementer for the build phase (see the matching mandatory-dispatch row in `process-build/SKILL.md`). This was advisory routing until 2026-07-10; it is now a mandatory-dispatch row per architecture-v2 Delta-1 close-out.
...
**You MUST also dispatch adversarial-reviewer** to challenge assumptions before committing. (Since the 2026-06-11 workflow adoption this dispatch is unconditional — the workflow script runs it on every plan; the old high-stakes-only gating is retired. The fallback prose path must match the script.)
