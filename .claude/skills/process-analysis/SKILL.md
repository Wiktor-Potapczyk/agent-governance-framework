...
- Task-classifier returned `TASK TYPE: Analysis`
- Evaluating an artifact against a rubric (Evaluation mode)
- Investigating a cause, tracing a failure, reasoning through "why" or "how" (Investigation mode)
- Breaking a Compound task into typed sub-tasks with dependencies (Decomposition mode)

## Do-NOT-use-when

- Single factual lookup with no reasoning chain — handle as Quick inline
- Designing a new system or sequencing future work — use `process-planning` instead
- Writing executable code or modifying files — use `process-build`
- Producing audience-facing prose — use `content-marketer` directly (not via process-research)

## Gotchas

- **Mode confusion** — Evaluation needs an existing rubric (define one first if missing); Investigation needs multi-step reasoning (if it's one fact, it's Quick); Decomposition is one level deep only — if a sub-task looks Compound, flatten it.
- **Synthesis is mandatory if 2+ specialist agents dispatched** — `research-synthesizer` resolves contradictions; skipping it is a process violation caught by the Stop hook.
- **Don't pre-judge results in agent prompts** — state observable facts only; the bias-guard hook will block prompts containing hypotheses or proposed conclusions.
...
| Code architecture, SOLID, coupling, layering | architect-reviewer |
...
| n8n workflow logic, branching, error recovery | n8n-workflow-architect |
