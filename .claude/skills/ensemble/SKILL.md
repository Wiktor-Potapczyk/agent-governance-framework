...
Run 4 agents with distinct thinking lenses in parallel on a single framing or design question; main session frames, agents answer, main session synthesizes the divergence map. Value is in surfacing perspectives that a single reasoning chain would miss, not in finding consensus.
...
- Multi-step builds where sequential agent chains depend on each other's output — that's not divergence, it's pipeline; use `process-build` decomposition

## Gotchas

- **All 4 agents MUST spawn in a single message** — output text between calls or sequential dispatch makes them run sequentially, defeating the divergence-map purpose.
- **Don't answer the lenses inline** — main session frames the question; agents answer; main session synthesizes the divergence. Inline-answering corrupts the divergence map with main-session bias.
- **Agreement is also signal** — when all 4 lenses agree, the answer is likely robust; when they sharply disagree, the disagreement is the finding (not a problem to resolve).
- **Question must be de-biased before dispatch** — anchored hypotheses in the framing make all 4 lenses validate-not-investigate. Strip "I think X, what do you think?" framings.
