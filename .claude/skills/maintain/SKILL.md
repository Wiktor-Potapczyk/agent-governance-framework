...
- User says `/maintain`, "clean up files", "organize work directory", "archive stale files"
- After a major project milestone — work files have accumulated and need triage
- Project's `work/` directory has 10+ files and signal-to-noise feels low

## Do-NOT-use-when

- Vault-wide cleanup is the goal — use `/vault-maintain` (different scope: tag hygiene, MOC freshness, vault-level)
- Project has fewer than ~5 work files — overhead of agent dispatch exceeds benefit; sort manually
- Looking for a specific known file — use Glob/Grep directly

## Gotchas

- **Never read work files inline in main session** — context budget gets trashed; ALWAYS delegate to a sub-agent.
- **No auto-deletes** — moves to `work/archive/` are the only allowed destructive op; "obvious junk" today often becomes load-bearing context tomorrow.
- **Classification rules are the spec** — pass the full keep/archive/merge rule set to the agent prompt; agents that improvise classification produce inconsistent triage.
