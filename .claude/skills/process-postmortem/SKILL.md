...
description: Use AFTER something failed in production or testing — an n8n workflow silently dropped data, a database migration broke, an automation produced wrong output, a hook misfired, a deploy regressed. Produces a dated root-cause artifact (timeline → proven root cause → contributing factors → prevention items) and feeds the prevention items back into memory/hooks. NOT a pre-ship quality gate (that's process-qa / process-pentest) and NOT a velocity retro (no installed skill covers that since the gstack set was archived 2026-09-14). Triggers — postmortem, what went wrong, incident, why did X break, root cause.
...
- Weekly velocity / commit-pattern retrospective → no installed skill since the gstack set was archived on 2026-09-14 (it had no runtime on this machine); `pm-execution:retro` covers sprint retros only
...
- **The silent-failure check is mandatory.** For every failure, ask: did anything show green while being wrong? Silent success is the vault's documented worst failure class ([[agents-unsupervised-failure-modes]] #1). Name where the green-but-wrong signal was.
...
- **Doctrine / blocking / destructive** (CLAUDE.md edit, blocking hook, asset removal) → stage for Wiktor; do not auto-apply.
...
- Surface the Wiktor-gated prevention items explicitly; do not silently apply them.
...
Prevention — Wiktor-gated (staged): [bulleted, with why gated]
