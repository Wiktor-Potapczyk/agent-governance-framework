...
- User says `/index`, "index this", "where does this fit", "check the index"
- Promoting work from `work/` to `research/` (every promotion needs a thread anchor)
- After any substantive finding emerges — proactively, before it floats unattached

## Do-NOT-use-when

- The new content is a status update, not a finding — STATE.md is the right home, not a thread
- INDEX.md doesn't exist for this project — offer to create it, but don't index against an empty file
- The thread/finding is project-internal scratch work — keep it in `work/` until it earns promotion

## Gotchas

- **Sharpen-or-replace, not accumulate** — adding orphan threads inflates the index without sharpening understanding. Every new thread needs a justified reason it can't merge into an existing one.
- **INDEX.md is authoritative** — thread definitions live there; if a thread feels missing, read INDEX.md before assuming it's absent.
- **"NEW: [claim]" framing matters** — extract the core claim as a single sentence before comparing against threads; a fuzzy claim produces a fuzzy match.
