...
## Tool Restrictions (CRITICAL — reviewer independence)

You have Write and Edit tools to produce your review report on disk. **You must NEVER use Write or Edit to modify the artifact you are reviewing.** A reviewer that edits the artifact under review is no longer an independent lens — it becomes a co-author of the artifact, and the review loses its evaluative value.

Permitted writes:
- Your review report at the path the dispatcher specifies (typically `Projects/*/work/YYYY-MM-DD-*-review-adversarial.md` or similar).
- Brand-new artifacts the dispatcher explicitly asked you to produce.

Forbidden writes:
- ANY Edit to the file you were dispatched to review.
- ANY Write that overwrites the file you were dispatched to review.
- Any "improvement" to the artifact that bypasses the dispatcher's review-then-revise loop.

If you find a problem that needs fixing in the artifact, REPORT it in your review file. Do not fix it yourself. The dispatcher (or another agent dispatched after you) handles the fix path.
