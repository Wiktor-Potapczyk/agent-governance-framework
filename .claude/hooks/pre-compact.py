...
VAULT = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "..")
...
# --- Discover projects once, feed both sections ---
# Was two independent flat os.listdir passes. One bounded depth-2 scan now serves
# both, so a nested project cannot appear in one section and be missing from the
# other. Section labels carry the full relative identity (`Project/Module-8B`),
# which is the point: a bare leaf is ambiguous in the recovery snapshot.
...
try:
    ...
    # Observability for the total-loss case: an import/call failure here empties
    # the ENTIRE pre-compaction snapshot (states + plans) with no trace, at the
    # moment state loss is most expensive. Fail-open is preserved (projects=[]),
    # this only adds a record of why. Never allowed to raise past this block.
    try:
        from _governance_logger import log_fire, session_from
        log_fire("pre-compact", decision="degraded",
                 detail="_project_discovery unavailable: %s" % discovery_error,
                 session=session_from({"transcript_path": transcript_path}))
    except Exception:
        pass

# --- Collect STATE.md files ---
states_section = ""
for proj, state_path, _plan_path in projects:
    try:
...
plans_section = ""
for proj, _state_path, plan_path in projects:
    if not plan_path:
        continue
    try:
...
if discovery_error:
    recovery += f"[SNAPSHOT DEGRADED: project discovery failed, {discovery_error}]\n\n"
...
# Contract C1. C1 offers state-writer hooks an exemption from fire logging; taken
# here anyway because the record doubles as the only measurement of how often this
# session compacts, and a PreCompact fire is rare enough to cost nothing.
try:
    sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
    from _governance_logger import log_fire, session_from
    log_fire("pre-compact", decision="snapshot",
             detail="bytes=%d" % len(recovery),
             session=session_from({"transcript_path": transcript_path}))
except Exception:
    pass
...
# --- H-2 stdout block REMOVED 2026-05-10 ---
# Empirical finding from 2026-05-10 compact: PreCompact JSON output schema does NOT
# accept hookSpecificOutput.additionalContext (only PreToolUse / UserPromptSubmit /
...
# See: reference_compact_hooks_no_additionalcontext.md
