...
def _gov_log_path():
    ...
    somewhere the test does not look. A first attempt at this fix did exactly
    that and broke three tests in test_verifier_gate_check.py.
...
def stamp_and_append(entry, log_path, correlation_id):
    ...
    try:
        ...
        print(f"WARN: dispatch-compliance could not write its governance event ({type(exc).__name__}: {exc})", file=sys.stderr)
...
def main():
    ...
    # Read the tail up to the start of the turn (2026-09-21, silent-failure
    # review item 1: a fixed 200 KB tail could not see a turn behind one large
    # tool result). Falls back to the old fixed tail if the helper is missing.
    window_info = {"window_bytes": 0, "capped": False}
    try:
        from _turn_boundary import tail_to_turn_start
        lines, window_info = tail_to_turn_start(transcript_path, start_bytes=READ_BYTES)
    except Exception as exc:
        print(f"WARN: dispatch-compliance could not use _turn_boundary ({type(exc).__name__}: {exc}); "
              f"reading the fixed {READ_BYTES} byte tail instead.", file=sys.stderr)
        ...
        with open(transcript_path, "r", encoding="utf-8", errors="replace") as f:
            ...
            tail = f.read()
        lines = tail.split("\n")
        window_info = {"window_bytes": read_bytes, "capped": False, "fallback": True}
    ...
    contract_ever_found = False  # review A5: the sidecar fallback must not re-arm on a later TASK TYPE mention
    ...
    for line in lines:
        ...
        # Text before tools (review A4, 2026-09-21): a dispatch that precedes
        # its classification in the same message belongs to that classification.
        for block in blocks:
            if block.get("type") == "text":
                # Pure logic decides whether this block changes state.
                state = scan_assistant_text_block(str(block.get("text", "")), state)
                if state["found_contract"]:
                    contract_ever_found = True
        for block in blocks:
    ...
    if not state["found_contract"] and not contract_ever_found and recent_process_skill and _SIDECAR_AVAILABLE:
        ...
        except Exception as exc:  # silent-failure review item 11: say so
            print(f"WARN: dispatch-compliance sidecar fallback failed ({type(exc).__name__}: {exc}); "
                  f"the contract for {'+'.join(trackable_skills_seen)} was not checked.", file=sys.stderr)
            try:
                from datetime import datetime
                from _governance_logger import session_from
                stamp_and_append({
                    "ts": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                    "event": "h11_fallback_error",
                    "hook": "dispatch-compliance",
                    "session": session_from(payload),
                    "skill": "+".join(trackable_skills_seen),
                    "error": f"{type(exc).__name__}: {exc}",
                    "schema": 2,
                }, _gov_log_path(), correlation_id)
            except Exception:
                pass
    ...
    unrecognised_declared = list(state.get("unrecognised_declared") or [])
    if unrecognised_declared:
        print(f"WARN: MUST DISPATCH names {', '.join(unrecognised_declared)}, which match no known agent or skill; "
              f"the obligation on them cannot be checked.", file=sys.stderr)
    _telemetry = {
        "task_type": task_type_str,
        "window_bytes": window_info.get("window_bytes", 0),
        "window_capped": bool(window_info.get("capped")),
        "unrecognised_declared": unrecognised_declared,
    }
    ...
    if missing:
        ...
        try:
            ...
            stamp_and_append({
                ...
                # review A12: direct (the name itself, Skill/Workflow/Agent) or alias
                "matched_by": {item: ("direct" if item in dispatched else "alias") for item in must_dispatch},
                "schema": 2,
                **_telemetry,
            }, log_path, correlation_id)
        except Exception:
