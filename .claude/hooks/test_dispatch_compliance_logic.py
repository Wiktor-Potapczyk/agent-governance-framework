...
class TestFindTaskType:
    ...
    def test_an_unrecognised_task_type_is_returned_for_the_caller_to_name(self):
        """Flipped 2026-09-21 (harness audit 1, review A6): a misspelt value used
        to read as no classification and silently disabled three gates."""
        assert find_task_type("TASK TYPE: Banana") == "banana"
...
class TestComputeMissing:
    ...
    def test_a_declared_process_skill_is_not_discharged_by_its_agent_alias(self):
        """Adversarial build review of the QA contract (2026-09-21, N7): the
        alias process-qa -> debugger let a debugger dispatch satisfy a declared
        QA obligation with no QA. The agent alias still says what process-qa may
        dispatch (agent-dispatch-check); it no longer says what counts as having
        run it. Only the Skill or Workflow invocation does."""
        assert compute_missing(["process-qa"], {"debugger"}) == ["process-qa"]
        assert compute_missing(["process-pentest"], {"debugger"}) == ["process-pentest"]
        assert compute_missing(["process-qa"], {"process-qa"}) == []
        assert compute_matched_alias_aware(["process-qa"], {"debugger"}) == []
...
class TestScanAssistantTextBlock:
    ...
    def test_new_classification_resets_previous(self):
        ...
        # 2026-09-21 (architect review of the audit-1 build, finding 1): a bare
        # "TASK TYPE: Quick" phrase in prose is an incidental mention and keeps
        # the contract; a real Quick block carries IMPLIES and JUSTIFICATION.
        kept = scan_assistant_text_block("TASK TYPE: Quick", state)
        assert kept["dispatched"] == {"a", "b"} and kept["must_dispatch"] == ["old"]
        text = "IMPLIES: a one-line answer\nTASK TYPE: Quick\nJUSTIFICATION: nothing to dispatch"
        result = scan_assistant_text_block(text, state)
