...
class DispatchComplianceBoundaryTests(unittest.TestCase):
    ...
    def test_fp_process_skill_alias_members_satisfy_silent(self):
        """Flipped 2026-09-21 (harness audit 1, review A2). Declaring a process
        skill is discharged only by invoking that skill (Skill or Workflow);
        dispatching its member agents is what the skill does, not what counts
        as having run it. Until today this test asserted the bypass."""
        self.assertEqual(
...
def test_pm_is_a_trackable_skill_for_the_h11_fallback():
    # Owner-ruled fix 2026-08-31 (O5 finding): a Skill-tool dispatch of 'pm'
    # must arm recent_process_skill so pm/DISPATCHES.json is actually read.
    ...
    assert is_trackable_process_skill("process-qa") is False  # terminal stays out
    assert is_trackable_process_skill("impeccable") is False


# --- H11 fallback contract-union (architect finding 2026-08-31) ---------------
