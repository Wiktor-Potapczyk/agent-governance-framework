...
Design: Projects/Agent-Governance-Research/work/2026-05-10-h4-design.md (status #approved).
...
# 2026-09-21: the verdict fields. The compact relay (ruled 2026-09-11) puts
# PASS, FAIL and Untested on one line separated by pipes; the block form puts
# them on three lines. Both are read by the same three patterns.
PATTERN_QA_COUNTS = re.compile(r'^\s*PASS:\s*(\d+)\s*/\s*(\d+)', re.IGNORECASE | re.MULTILINE)
PATTERN_QA_FAIL = re.compile(r'(?:^|\|)\s*FAIL:\s*(.*?)\s*(?=\|\s*Untested:|$)', re.IGNORECASE | re.MULTILINE)
PATTERN_QA_UNTESTED = re.compile(r'(?:^|\|)\s*Untested:\s*(.*?)\s*(?=\|\s*Verifier:|$)', re.IGNORECASE | re.MULTILINE)
PATTERN_RELAY_POINTER = re.compile(r'QA SCOPE:\s*\d+\s*claims?,\s*detail in\s+`?([^\s`]+?)`?(?=[\s,;)]|$)', re.IGNORECASE)
# The null phrases: 'none', 'none deliberately', 'none deliberately (reason)',
# and the workflow's own 'named no untested path and gave no reason'.
UNTESTED_NULL_RE = re.compile(r'^(?:none\b.*|.*gave no reason.*)$', re.IGNORECASE | re.DOTALL)
UNTESTED_NULL = {'', 'none', 'none deliberately', 'none deliberately.', 'none.'}
# Items that describe the verifier, not a surface (architect review N2, 2026-09-21:
# 'I checked these myself' and 'no independent check (...)' landed in a plan as tasks).
UNTESTED_SKIP_RE = re.compile(r'^(?:i|we)\b|^no independent\b|\b(?:checked|verified|tested)\s+(?:these|this|it|them)\s+(?:myself|ourselves)\b', re.IGNORECASE)
MAX_ROUTED_ITEMS = 6  # a relay is a list of paths, not a prose dump; matches the scope node's cap
UNTESTED_HEADING = '## QA untested surface'
...
def parse_qa_verdict(assistant_text: str):
    """Read the QA REPORT block into {passed, total, fail, untested}, or None
    when there is no report or no PASS line. `fail` and `untested` are the
    raw field texts ('none' when the field is absent)."""
    if not PATTERN_QA_REPORT.search(assistant_text):
        return None
    qa = extract_qa_block(assistant_text)
    counts = PATTERN_QA_COUNTS.search(qa)
    if not counts:
        return None
    fail = PATTERN_QA_FAIL.search(qa)
    untested = PATTERN_QA_UNTESTED.search(qa)
    return {
        "passed": int(counts.group(1)),
        "total": int(counts.group(2)),
        "fail": (fail.group(1).strip() if fail else "none"),
        "untested": (untested.group(1).strip() if untested else "none"),
    }


def detected_qa_pass(assistant_text: str) -> bool:
    """A report is a PASS only when every claim passed and the FAIL line is
    none. Until 2026-09-21 this matched any `PASS: n / N` line without
    comparing n to N, so a report of 0 / 9 ticked the task box; CLAUDE.md's
    Task Plan rule says a FAIL adds a note and leaves the entry open. The
    table form (| OVERALL | ... | PASS) is kept for the older report shape."""
    if not PATTERN_QA_REPORT.search(assistant_text):
        return False
    v = parse_qa_verdict(assistant_text)
    if v is None:
        # The older table shape carries no PASS: n / N line; it is a pass on
        # its own only when no count line contradicts it (build review N2).
        return bool(PATTERN_QA_PASS_TABLE.search(assistant_text))
    if v["total"] <= 0 or v["passed"] != v["total"]:
        return False
    return v["fail"].strip().lower().rstrip(".") in ("", "none")


def untested_items(assistant_text: str):
    """The Untested field split into items (semicolons separate them). The
    null phrases yield an empty list."""
    v = parse_qa_verdict(assistant_text)
    if v is None:
        return []
    raw = v["untested"].strip()
    if raw.lower() in UNTESTED_NULL or UNTESTED_NULL_RE.match(raw):
        return []
    return [part.strip() for part in raw.split("; ") if part.strip() and not UNTESTED_SKIP_RE.search(part.strip())]


def relay_project_plan(assistant_text: str):
    """The task_plan.md of the project the compact relay names, or None when
    the reply carries no relay pointer. Only a relay arms the untested router:
    a quoted report in prose is not a QA turn (build review N12)."""
    m = PATTERN_RELAY_POINTER.search(assistant_text)
    if not m:
        return None
    pointer = m.group(1).replace("\\", "/").strip("`'\"")
    parts = [p for p in pointer.split("/") if p]
    if "work" in parts:
        parts = parts[:parts.index("work")]
    if parts and parts[0] == "Projects":
        parts = parts[1:]
    if not parts:
        return None
    plan = Path(PROJECTS_DIR).joinpath(*parts) / "task_plan.md"
...
def _execute_fail_note(match: dict, assistant_text: str) -> bool:
    """A QA report that is not a PASS notes the failure on the open entry and
    leaves the box open (CLAUDE.md Task Plan rule). Same undo, apply and
    verify path as the tick; deduplicated on the note text."""
    v = parse_qa_verdict(assistant_text)
    if v is None:
        return False
    note = f"QA FAIL {v['passed']} / {v['total']}: {v['fail'][:SUMMARY_MAX]}"
    if note in match["original_line"]:
        return False
    line_hash = hashlib.sha256((match["original_line"] + note).encode()).hexdigest()[:16]
    if is_recent_duplicate(line_hash):
        return False
    replacement_line = match["original_line"].rstrip() + f"  ← {note}"
    if DRY_RUN:
        log(f"DRY_RUN: would note '{note}' on {match['file'].name} line {match['line_index']+1}")
        return False
    try:
        write_undo_entry(match, replacement_line)
        apply_replacement(match, replacement_line)
    except Exception as e:
        log(f"fail note skipped: {e}")
        return False
    if not verify_write(match, replacement_line):
        revert_via_undo(match)
        return False
    record_dedup(line_hash)
    log(f"NOTED FAIL {match['file'].name} line {match['line_index']+1} task={match['task_id']}")
    return True


def route_untested(plan_path, items, today=None) -> int:
    """Append each untested item as an open task under UNTESTED_HEADING in the
    project's task_plan.md, so an Untested line is a work item and not only a
    disclosure (adversarial review of the QA contract, finding 18: Production
    had 91 QA entries and zero 'untested' in its plan). Items already present
    in the plan, or routed within the dedup window, are skipped. Returns the
    number of lines added."""
    if not items:
        return 0
    today = today or datetime.now().strftime("%Y-%m-%d")
    try:
        # newline="" keeps the file's own line endings (architect review N3:
        # translated reads plus newline="" writes flattened CRLF plans).
        with open(plan_path, "r", encoding="utf-8", newline="") as fh:
            body = fh.read()
    except Exception as e:
        log(f"route_untested read failed for {plan_path}: {e}")
        return 0
    nl = "\r\n" if "\r\n" in body else "\n"
    items = [i for i in items if i and i.strip()]
    if len(items) > MAX_ROUTED_ITEMS:
        log(f"route_untested: {len(items)} items in the relay, routing the first {MAX_ROUTED_ITEMS} ({len(items) - MAX_ROUTED_ITEMS} more dropped)")
        items = items[:MAX_ROUTED_ITEMS]
    new_lines = []
    for item in items:
        item = item.strip()
        if not item or item.lower() in UNTESTED_NULL:
            continue
        if item in body:
            continue
        h = hashlib.sha256(("untested:" + item).encode()).hexdigest()[:16]
        if is_recent_duplicate(h):
            continue
        new_lines.append((h, f"- [ ] QA untested ({today}): {item}"))
    if not new_lines:
        return 0
    if DRY_RUN:
        log(f"DRY_RUN: would route {len(new_lines)} untested item(s) into {plan_path}")
        return 0
    block = nl.join(l for _, l in new_lines)
    try:
        write_undo_entry({"file": Path(plan_path), "line_index": -1, "original_line": "", "task_id": "qa-untested"}, block)
    except Exception as e:
        log(f"route_untested: undo log write failed; mutation skipped for safety: {e}")
        return 0
    if UNTESTED_HEADING + nl in body:
        addition = block + nl
        head, sep, tail = body.partition(UNTESTED_HEADING + nl)
        # insert right after the heading's existing items: find the end of the section
        section, nxt, rest = tail.partition(nl + "## ")
        section = section.rstrip("\r\n") + nl + addition
        new_body = head + sep + section + (nxt + rest if nxt else "")
    else:
        new_body = body.rstrip("\r\n") + nl + nl + UNTESTED_HEADING + nl + block + nl
    try:
        tmp = str(plan_path) + f".tmp.{os.getpid()}"
        with open(tmp, "w", encoding="utf-8", newline="") as f:
            f.write(new_body)
        os.replace(tmp, plan_path)
    except Exception as e:
        log(f"route_untested write failed for {plan_path}: {e}")
        return 0
    try:
        with open(plan_path, "r", encoding="utf-8", newline="") as fh:
            check = fh.read()
        ...
        check = ""
    if not all(l in check for _, l in new_lines):
        log(f"route_untested: write verify failed for {plan_path}; restoring the previous body")
        try:
            tmp = str(plan_path) + f".tmp.{os.getpid()}"
            with open(tmp, "w", encoding="utf-8", newline="") as f:
                f.write(body)
            ...
            log(f"route_untested: restore failed: {e}")
        return 0
    for h, _ in new_lines:
        record_dedup(h)
    log(f"ROUTED {len(new_lines)} untested item(s) into {Path(plan_path).name}")
...
def main():
    ...
    verdict = parse_qa_verdict(assistant_text)
    is_pass = detected_qa_pass(assistant_text)
    if verdict is None and not is_pass:
        return  # Not a QA turn

    # 2026-09-21: an Untested line becomes work. Routed into the active
    # project's plan before the task-id logic, which may return early.
    items = untested_items(assistant_text)
    if items:
        _u_plan = relay_project_plan(assistant_text)
        if _u_plan is not None:
            route_untested(_u_plan, items)

    if not is_pass:
        # A failed or partial report: note the entry, never tick it.
        qa_block_f = extract_qa_block(assistant_text)
        scope_f = PATTERN_SCOPE.search(qa_block_f)
        ids_f = list(dict.fromkeys(PATTERN_TASK_ID.findall(scope_f.group(1)))) if scope_f else []
        if not ids_f:
            ids_f = list(dict.fromkeys(PATTERN_TASK_ID.findall(assistant_text)))
        if ids_f:
            _f_plans = find_task_plans(detect_active_project())
            _f_match = regex_match(ids_f[:1], _f_plans) if _f_plans else None
            if _f_match is not None:
                _execute_fail_note(_f_match, assistant_text)
        return
...
def selftest():
    ...
    Cases:
      ...
      T-SM-2   (existing, multi-char prefix + digit) — PRJ21-CLEANUP-BUILD
    ...
    cases = [
        ...
        ("T-SM-2",   "Applied **PRJ21-CLEANUP-BUILD** to live KNBOXfMZmx7EPPxl.",           "PRJ21-CLEANUP-BUILD"),
