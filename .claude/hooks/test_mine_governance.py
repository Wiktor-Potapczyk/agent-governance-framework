...
# Bound lazily rather than by `from mine_governance import (...)`. A from-import
# resolves every name at COLLECTION time, so a disarmed or renamed hook kills the
# whole module before one assertion runs, and the disarm probe cannot tell a
# protected guard from an unprotected one. See _hooktest.bind.
import importlib

from _hooktest import bind

_mg = importlib.import_module("mine_governance")

DEFAULT_C = bind(_mg, "DEFAULT_C")
DEFAULT_D = bind(_mg, "DEFAULT_D")
HIGH_SEV_C = bind(_mg, "HIGH_SEV_C")
WINDOW_DAYS = bind(_mg, "WINDOW_DAYS")
mine = bind(_mg, "mine")
_sig_id = bind(_mg, "_sig_id")
_sig_key = bind(_mg, "_sig_key")
_normalize_reason = bind(_mg, "_normalize_reason")
...
class SyntheticFixtureTests(unittest.TestCase):
    ...
    # ------------------------------------------------------------------
    # CASE G2 (owner ruling 2026-09-06, R19): accepted-as-designed is a
    # PERMANENT suppression. The same blueprint-mode sig that regresses
    # under resolution "hook-tuned" must stay absent when its latest ledger
    # entry carries resolution "accepted-as-designed", no matter how much
    # post-resolved volume clears the recurrence gate.
    # ------------------------------------------------------------------
    def test_accepted_as_designed_suppresses_permanently(self):
        nr = _normalize_reason("REVIEWER SCOPE: Edit blocked")
        key = _sig_key("reviewer_scope_violation", "blueprint-mode", "reviewer-scope-violation-check", nr)
        sid = _sig_id(key)
        with tempfile.NamedTemporaryFile(mode="w", suffix=".jsonl", delete=False, encoding="utf-8") as tf:
            tf.write(json.dumps({
                "sig_id": sid,
                "resolved_ts": "2026-05-17",
                "resolution": "accepted-as-designed",
                "note": "permanent suppression test",
            }) + "\n")
            ledger_path = tf.name
        try:
            results = mine(_FIXTURE_LOG, _NOW, window_days=WINDOW_DAYS, resolved_ledger_path=ledger_path)
            rec = self._find(results, event_label="reviewer_scope_violation", agent_type="blueprint-mode")
            self.assertIsNone(rec, "accepted-as-designed sig must stay suppressed despite post-resolved volume")
        finally:
            os.unlink(ledger_path)
...
# Plan: Projects/Agent-Governance-Research/work/2026-08-17-hermes-p4-warn-event-miner-plan.md
...
class WarnMinerTests(_WarnMinerBase):
    ...
    def test_warns_accepted_as_designed_suppresses_permanently(self):
        # Owner ruling 2026-09-06 (R19): same permanent semantics on the warn tier.
        sid = self._push_sig_id()
        with tempfile.NamedTemporaryFile(mode="w", suffix=".jsonl", delete=False,
                                         encoding="utf-8") as tf:
            tf.write(json.dumps({"sig_id": sid, "resolved_ts": "2026-08-01",
                                 "resolution": "accepted-as-designed",
                                 "note": "warn permanent suppression"}) + "\n")
            ledger = tf.name
        try:
            res = self._run(ledger=ledger)
            self.assertIsNone(self._push_candidate(res),
                              "accepted-as-designed warn sig must stay suppressed past resolved_ts")
        finally:
            os.unlink(ledger)
