...
# 2026-09-21: the compact QA relay names the qa-log entry that holds the
# evidence; the hook opens that file. The vault root is hook_dir/../.. (the
# _hooktest isolation preserves that depth, so a test copy resolves into its
# own temp tree, never into the real vault).
VAULT_ROOT = Path(__file__).resolve().parent.parent.parent
RELAY_POINTER = re.compile(r'QA SCOPE:\s*(\d+)\s*claims?,\s*detail in\s+`?([^\s`]+?)`?(?=[\s,;)]|$)', re.IGNORECASE)
RELAY_COUNTS = re.compile(r'^\s*PASS:\s*(\d+)\s*/\s*(\d+)', re.IGNORECASE | re.MULTILINE)
QA_VERDICT = re.compile(r'(?:^|\n)\s*QA REPORT\s*[\n:]', re.IGNORECASE)
MAX_ENTRIES_SCANNED = 6  # today's entries checked newest first (concurrent sessions append)
NO_CLAIMS_LINE = re.compile(r'^\s*no verifiable claims\s*\.?\s*$', re.IGNORECASE | re.MULTILINE)
ENTRY_DATE = re.compile(r'^##\s*(\d{4}-\d{2}-\d{2})')
# True: an unbacked relay is a hard block. False: stderr warning and event only.
# The switch exists so the gate can be lowered without an edit to the check
# itself; the polarity is pinned by test_process_step_check.py.
RELAY_BACKED_BLOCK = True
...
def check_scope(process_skill, text_blocks):
    ...
    if process_skill == "process-qa":
        for text in text_blocks:
            if NO_CLAIMS_LINE.search(strip_fences(text)):
                return True, ""
...
def check_qa_report(process_skill, text_blocks):
    ...
    for text in text_blocks:
        if "QA REPORT" in text and re.search(r'(?:PASS|FAIL)', text):
            return True, ""
    # Allow graceful exit when no verifiable claims. The phrase must stand on
    # its own line (the skill says so); until 2026-09-21 a bare substring match
    # let any sentence containing it disarm the gate.
    for text in text_blocks:
        if NO_CLAIMS_LINE.search(strip_fences(text)):
            return True, ""
...
def _resolve_qa_log(pointer, root):
    """The relay's path may be vault-relative or omit the Projects/ prefix."""
    p = pointer.replace("\\", "/").strip("`'\"")
    for cand in (Path(root) / p, Path(root) / "Projects" / p):
        if cand.is_file():
            return cand
    return None
...
def _last_log_entry(text):
    parts = _log_entries(text)
    return parts[-1] if parts else ""


def _final_assistant_texts(lines):
    """Text blocks of the assistant entries after the last real user message
    (a tool_result wrapper is not a user message)."""
    texts = []
    for raw in lines:
        raw = raw.strip()
        if not raw:
        ...
        if entry.get("type") == "user":
            ...
            real = isinstance(content, str) or (isinstance(content, list) and any(
...
def _emit_relay_unbacked(payload, msg):
    """Telemetry for the relay gate (architect review N4, 2026-09-21: the gate
    shipped ON with no event to evaluate its false-block rate by)."""
    try:
        from _event_emit import emit_event
        from _governance_logger import session_from
        emit_event(
            event="qa_relay_unbacked",
            hook="process-step-check",
            session=session_from(payload),
            extra={"reason": msg, "blocked": bool(RELAY_BACKED_BLOCK)},
        )
    except Exception:
        pass  # instrumentation must never break the gate


def check_qa_log_backing(process_skill, text_blocks, root=None):
    """HARD: a compact QA relay must be backed by the qa-log entry it names.

    The 2026-09-11 ruling moved the QA SCOPE and QA REPORT blocks out of the
    reply into work/qa-log.md and left three lines in the reply. Nothing then
    read the file, so the three lines could be typed without any QA having
    happened (adversarial review of the QA contract, 2026-09-21, finding 8).
    Now: the file must exist, its last entry must be dated today or
    yesterday, and its PASS n / N must equal the relay's. A reply that carries
    the full blocks (no pointer) is left to check_qa_report as before."""
    if process_skill != "process-qa":
        return True, ""
    raw = "\n".join(text_blocks)
    joined = "\n".join(strip_fences(t) for t in text_blocks)
    m = RELAY_POINTER.search(joined)
    if not m:
        if RELAY_POINTER.search(raw):
            return False, "the QA relay is fenced (inside a code block), where no hook can read it; relay it as plain text"
        if QA_VERDICT.search(joined) and RELAY_COUNTS.search(joined):
            # Build review N5: a verdict with no pointer armed nothing. Since the
            # 2026-09-11 ruling the blocks live in work/qa-log.md; the reply must
            # name the log so the verdict can be checked against it.
            if NO_CLAIMS_LINE.search(joined):
                return True, ""
            return False, "a QA verdict on the process-qa path must name its qa-log (QA SCOPE: N claims, detail in <project>/work/qa-log.md)"
        return True, ""
    n_claims, pointer = int(m.group(1)), m.group(2)
    counts = RELAY_COUNTS.search(joined)
    if not counts:
        return False, "Compact QA relay carries no PASS: n / N line"
    log = _resolve_qa_log(pointer, root or VAULT_ROOT)
    if log is None:
        return False, f"QA relay points at {pointer}, which does not exist under the vault"
    try:
        entries = _log_entries(log.read_text(encoding="utf-8", errors="replace"))
    except OSError as exc:
        return False, f"QA log {pointer} could not be read: {exc}"
    today = datetime.date.today().isoformat()
    todays = [e for e in entries[-MAX_ENTRIES_SCANNED:] if ENTRY_DATE.match(e.strip()) and ENTRY_DATE.match(e.strip()).group(1) == today]
    if not todays:
        last = ENTRY_DATE.match(entries[-1].strip()) if entries else None
        return False, f"{pointer} has no entry dated today (its last entry is dated {last.group(1) if last else 'nothing'})"
    seen = []
    for entry in reversed(todays):
        if "QA REPORT" not in entry:
            continue
        # Every PASS line of the entry counts (architect review N5: an entry that
        # records a self-check and an independent rerun carries two).
        for lc in RELAY_COUNTS.finditer(entry):
            seen.append(f"{lc.group(1)} / {lc.group(2)}")
            if (lc.group(1), lc.group(2)) == (counts.group(1), counts.group(2)) and int(lc.group(2)) == n_claims:
                return True, ""
    return False, (f"the relay says PASS: {counts.group(1)} / {counts.group(2)} over {n_claims} claims but today's entries of "
                   f"{pointer} report {', '.join(seen) if seen else 'no QA REPORT with a PASS line'}")
...
def main():
    ...
    # 2026-09-21: the compact QA relay is checked from the final assistant text
    # on every turn: whatever process skill ran last (architect review N6) and
    # whether or not the skill invocation is inside the 200 KB window (measured
    # on a real turn: 1.9 MB of tool output between Skill(process-qa) and the
    # reply, so found_skill was False and the relay went unread).
    _relay_texts = _final_assistant_texts(lines)
    _relay_joined = "\n".join(_relay_texts)
    if RELAY_POINTER.search(_relay_joined) or (QA_VERDICT.search(strip_fences(_relay_joined)) and RELAY_COUNTS.search(strip_fences(_relay_joined))):
        ok, msg = check_qa_log_backing("process-qa", _relay_texts)
        if not ok:
            _emit_relay_unbacked(payload, msg)
            if RELAY_BACKED_BLOCK:
                print(json.dumps({"decision": "block", "reason": f"PROCESS STEP CHECK (process-qa relay): {msg}. The relay must match the qa-log entry it names."}))
                return
            print(f"WARN: QA relay not backed by its log: {msg}", file=sys.stderr)

    if not last_process_skill or not found_skill:
        return  # No process skill invoked this turn (inside the window) — nothing else to check
    ...
    passed, msg = check_qa_log_backing(last_process_skill, text_after_skill)
    if not passed:
        _emit_relay_unbacked(payload, msg)
        if RELAY_BACKED_BLOCK:
            hard_failures.append(msg)
    ...
    soft_warnings = []
    if 'soft_warnings_pre' in dir():
        soft_warnings.append(soft_warnings_pre)
