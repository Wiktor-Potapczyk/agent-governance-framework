...
Spec: Projects/Agent-Governance-Research/work/archive/2026-06-07-governance-miner-spec.md
"""
...
# ---------------------------------------------------------------------------
# SESSION ADMISSION  (REV-7, 2026-07-30 — Wiktor-authorized)
# ---------------------------------------------------------------------------
...
def _load_resolutions(ledger_path: str) -> dict:
    """Return {sig_id: resolution} keeping the entry with the LATEST resolved_ts.

    Companion to _load_resolved (which stays untouched per REV-6). Added by
    owner ruling 2026-09-06 (R19): an "accepted-as-designed" resolution is a
    PERMANENT suppression, so the recurrence gate must know the resolution,
    not only the date."""
    result = {}
    latest = {}
    if not ledger_path or not os.path.isfile(ledger_path):
        ...
        with open(ledger_path, "r", encoding="utf-8") as f:
            for raw in f:
                ...
                try:
                    ...
                    continue
                sid = entry.get("sig_id", "")
                rts = (entry.get("resolved_ts", "") or "")[:10]
                if sid and (sid not in latest or rts > latest[sid]):
                    latest[sid] = rts
                    result[sid] = entry.get("resolution", "")
    except Exception:
        pass
    return result
...
def mine(
    ...
    resolutions = _load_resolutions(resolved_ledger_path)
    ...
    for sid, m in sig_meta.items():
        ...
        if resolved_date:
            if resolutions.get(sid) == "accepted-as-designed":
                # Owner ruling 2026-09-06 (R19): accepted-as-designed is
                # PERMANENT. By-design behavior recurring is not a regression.
                suppressed = True
            else:
...
def mine_warns(
    ...
    for sid, m in warn_meta.items():
        ...
        if resolved_date:
            if resolutions.get(sid) == "accepted-as-designed":
                # R19 (2026-09-06): permanent suppression, same as mine().
                suppressed = True
            else:
                post_count = m.get("post_resolved_count", 0)
                post_days = len(m.get("post_resolved_dates", set()))
