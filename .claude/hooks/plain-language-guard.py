...
record: .claude/rules/plain-language.md. Plan of record:
Projects/Agent-Governance-Research/work/2026-08-10-plain-language-standard-plan.md
...
def _main():
    ...
    if blocked:
        sys.stderr.write(
            ...
            f"Rewrite per .claude/rules/plain-language.md and retry the write.\n"
        )
    ...
    if total > 0 or hygiene:
        ...
        if total > 0:
            message += (
                ...
                f"concrete values (rules: .claude/rules/plain-language.md)."
            )
        if hygiene:
