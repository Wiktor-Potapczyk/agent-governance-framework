...
from _hooktest import bind  # lazy attribute binding, see _hooktest
SKILL_AGENT_ALIASES = bind(mod, "SKILL_AGENT_ALIASES")
ALWAYS_ALLOWED = bind(mod, "ALWAYS_ALLOWED")
extract_dispatch_names = bind(mod, "extract_dispatch_names")
KNOWN_DISPATCH_NAMES = bind(mod, "KNOWN_DISPATCH_NAMES")
...
class TestAliasMappingIntegrity(unittest.TestCase):
    ...
    # in the audit work at Projects/Agent-Governance-Research/work/
...
class TestEmitDispatchPartitionO13(unittest.TestCase):
    ...
    (registry agent names, local + plugin), closing the cross-field
    mislabel that put 611 of 931 unmatched skill-side join items into
    skill_context. Fail-open: a registry failure reverts to the old
    unpartitioned shape and never raises out of the emitter."""
    ...
    def test_overlap_name_agent_membership_wins(self):
        # n8n-reviewer is both a local agent and a local skill (the one
        # measured overlap member); agent membership wins by design so the
        # post-fix invariant skill_context-x-agent-names == empty holds by
        # construction.
    ...
    def test_live_registry_partitions_architect_reviewer(self):
        # Integration guard against registry schema drift: with the REAL
        # load_registry_agents, architect-reviewer is an agent name.
        mod._emit_dispatch("s", "x", ["architect-reviewer", "process-qa"],
                           False, False, "allow")
        extra = self._emitted_extra()
        self.assertIn("architect-reviewer", extra["agent_context"])
        self.assertNotIn("architect-reviewer", extra["skill_context"])
