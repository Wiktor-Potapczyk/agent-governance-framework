...
def _claude_md():
    ...
    Joining on the hook's VAULT_ROOT at module level means a disarmed or
    renamed hook raises during collection, pytest reports one error, zero
    assertions run, and the disarm probe cannot tell a protected guard from
    an unprotected one. Deferring it turns that into failing tests.
    """
    return os.path.join(bind(cmpc, "VAULT_ROOT"), "CLAUDE.md")
...
def test_cited_rule_addition_silent():
    err, rc = _run(_claude_md(), new_string=RULE_CITED)
    assert (err, rc) == ("", 0)


def test_uncited_rule_shaped_addition_warns():
    err, rc = _run(_claude_md(), new_string=RULE_UNCITED)
...
def test_warn_text_contains_no_dash_glyphs():
    err, _ = _run(_claude_md(), new_string=RULE_UNCITED)
    assert err
    assert not any(g in err for g in BANNED_GLYPHS)


def test_non_claude_md_path_out_of_scope():
    err, rc = _run("Projects/Agent-Governance-Research/work/CLAUDE-notes.md", new_string=RULE_UNCITED)
    assert (err, rc) == ("", 0)
...
def test_trivial_non_rule_edit_silent():
    err, rc = _run(_claude_md(), new_string="87 agents, 164 skills, across 34 installed plugins.")
    assert (err, rc) == ("", 0)


def test_memo_name_counts_as_provenance():
    text = "## New Rule\n**Always check twice.** Per feedback_no_push_without_asking.md.\n"
    err, rc = _run(_claude_md(), new_string=text)
    assert (err, rc) == ("", 0)
...
def test_write_payload_full_content_scanned():
    err, rc = _run(_claude_md(), content=RULE_UNCITED)
    assert rc == 0
    assert err != ""
...
def test_hook_cannot_emit_blocking_decision():
    ...
    _, rc = _run(_claude_md(), new_string=worst)
    assert rc == 0
