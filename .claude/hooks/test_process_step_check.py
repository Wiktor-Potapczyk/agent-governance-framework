...
class ProcessStepBoundaryTests(unittest.TestCase):
    ...
    def test_fp_qa_no_verifiable_claims_silent(self):
        ...
        A process-qa turn that declares 'no verifiable claims' on its own line must pass
        without a REPORT."""
        ok, _ = self.mod.check_qa_report("process-qa", ["Checked.\nno verifiable claims\n"])
        self.assertTrue(ok)

    def test_no_verifiable_claims_inside_a_sentence_does_not_disarm_the_gate(self):
        """Adversarial review of the QA contract (2026-09-21, finding 9): the exit
        phrase matched as a bare substring, so any prose containing it cleared the
        QA REPORT check. The skill says the phrase goes on its own line."""
        ok, _ = self.mod.check_qa_report(
            "process-qa", ["There were no verifiable claims worth listing today, so I skipped QA."])
        self.assertFalse(ok)
...
class TestQaLogBacking(unittest.TestCase):
    """2026-09-21: the compact three-line relay (ruled 2026-09-11) moved the QA
    evidence into work/qa-log.md, and no hook opened that file, so a typed relay
    naming a path that need not exist cleared every gate (adversarial review of
    the QA contract, finding 8). The relay is now verified against the entry it
    points at: the file exists, its last entry is dated today or yesterday,
    carries a QA REPORT, and its PASS n / N equals the relay's."""

    @classmethod
    def setUpClass(cls):
        cls.mod = load_hook()

    def _vault(self, entry_text):
        import tempfile, pathlib
        root = pathlib.Path(tempfile.mkdtemp(prefix="qa-backing-"))
        log = root / "Projects" / "Demo" / "work" / "qa-log.md"
    ...
    def _entry(self, passed, total, date=None):
        date = date or self._today()
        return (f"\n## {date} 10:00 local, demo build\n\nQA SCOPE\n"
                + "".join(f"- C{i}: claim {i}\n" for i in range(1, total + 1))
                + f"Source: tests\n\nQA REPORT\nPASS: {passed} / {total}\nFAIL: none\nUntested: none deliberately\n")

    def _relay(self, n, passed, total, path="Demo/work/qa-log.md"):
        return [f"QA SCOPE: {n} claims, detail in {path}\nQA REPORT:\nPASS: {passed} / {total} | FAIL: none | Untested: none"]

    def test_a_relay_backed_by_a_matching_entry_passes(self):
        root = self._vault(self._entry(3, 3))
        ok, msg = self.mod.check_qa_log_backing("process-qa", self._relay(3, 3, 3), root=root)
        self.assertTrue(ok, msg)

    def test_an_entry_with_two_pass_lines_backs_a_relay_on_either(self):
        """Architect review N5: three corpus entries record a self-check and an
        independent rerun in one entry; first-PASS-wins false-blocked them."""
        entry = self._entry(6, 6) + "\nIndependent rerun of the same six claims:\n\nQA REPORT\nPASS: 5 / 6\nFAIL: claim 4 refuted\nUntested: none deliberately\n"
        root = self._vault(entry)
        ok, msg = self.mod.check_qa_log_backing("process-qa", self._relay(6, 5, 6), root=root)
        self.assertTrue(ok, msg)
        ok, msg = self.mod.check_qa_log_backing("process-qa", self._relay(6, 6, 6), root=root)
        self.assertTrue(ok, msg)

    def test_the_relay_is_checked_when_another_process_skill_ran_last(self):
        """Architect review N6: with Skill(process-build) as the last process
        skill in the window, check_qa_log_backing returned True for it and the
        relay in the reply went unread."""
        import json as _json, os as _os, subprocess, sys as _sys, tempfile, pathlib
        from _hooktest import isolate
        tmp = pathlib.Path(tempfile.mkdtemp(prefix="qa-lastskill-"))
        hook = isolate("process-step-check.py", tmp)
        relay = self._relay(2, 2, 2, path="Nowhere/work/qa-log.md")[0]
        lines = [
            _json.dumps({"type": "user", "message": {"role": "user", "content": "go"}}),
            _json.dumps({"type": "assistant", "message": {"content": [
                {"type": "tool_use", "name": "Skill", "input": {"skill": "process-qa"}}]}}),
            _json.dumps({"type": "assistant", "message": {"content": [
                {"type": "tool_use", "name": "Skill", "input": {"skill": "process-build"}}]}}),
            _json.dumps({"type": "assistant", "message": {"content": [
                {"type": "text", "text": "BUILD SCOPE\nGoal: x\n"},
                {"type": "tool_use", "name": "Agent", "input": {"subagent_type": "architect-reviewer", "prompt": "review"}}]}}),
            _json.dumps({"type": "assistant", "message": {"content": [{"type": "text", "text": relay}]}}),
        ]
        tp = tmp / "transcript.jsonl"
        tp.write_text("\n".join(lines) + "\n", encoding="utf-8")
        gov = hook.parent / "governance-log.jsonl"
        env = dict(_os.environ, PYTHONIOENCODING="utf-8", GOVERNANCE_LOG_PATH=str(gov),
        ...
        # Architect review N4: the gate shipped ON with no event to evaluate it by.
        self.assertTrue(gov.exists(), "no governance log written")
        events = [_json.loads(l) for l in gov.read_text(encoding="utf-8").splitlines() if l.strip()]
        unbacked = [e for e in events if e.get("event") == "qa_relay_unbacked"]
        self.assertEqual(len(unbacked), 1, events)
        self.assertTrue(unbacked[0].get("blocked"))
        self.assertIn("does not exist", unbacked[0].get("reason", ""))

    def test_a_relay_pointing_at_a_missing_log_is_blocked(self):
        root = self._vault(self._entry(3, 3))
        ok, msg = self.mod.check_qa_log_backing(
            "process-qa", self._relay(3, 3, 3, path="Nowhere/work/qa-log.md"), root=root)
        self.assertFalse(ok)
        self.assertIn("does not exist", msg)

    def test_a_relay_whose_counts_differ_from_the_entry_is_blocked(self):
        root = self._vault(self._entry(2, 3))
        ok, msg = self.mod.check_qa_log_backing("process-qa", self._relay(3, 3, 3), root=root)
        self.assertFalse(ok)
        self.assertIn("PASS", msg)

    def test_a_relay_backed_only_by_a_stale_entry_is_blocked(self):
        root = self._vault(self._entry(3, 3, date="2026-01-02"))
        ok, msg = self.mod.check_qa_log_backing("process-qa", self._relay(3, 3, 3), root=root)
        self.assertFalse(ok)
        self.assertIn("dated", msg)

    def test_the_projects_prefix_is_optional_in_the_pointer(self):
        root = self._vault(self._entry(3, 3))
        ok, msg = self.mod.check_qa_log_backing(
            "process-qa", self._relay(3, 3, 3, path="Projects/Demo/work/qa-log.md"), root=root)
        self.assertTrue(ok, msg)

    def test_the_block_form_without_a_pointer_is_blocked(self):
        """Build review N5: the check armed only on the compliant format, so full
        blocks inline with no pointer cleared every gate with no file on disk.
        A QA verdict on the process-qa path must name its qa-log."""
        root = self._vault("")
        ok, msg = self.mod.check_qa_log_backing(
            "process-qa", ["QA SCOPE\n- one\nSource: x\n\nQA REPORT\nPASS: 1 / 1\nFAIL: none\nUntested: none"], root=root)
        self.assertFalse(ok)
        self.assertIn("name", msg)

    def test_a_fenced_relay_is_blocked(self):
        root = self._vault(self._entry(2, 2))
        fenced = "```\n" + self._relay(2, 2, 2)[0] + "\n```"
        ok, msg = self.mod.check_qa_log_backing("process-qa", [fenced], root=root)
        self.assertFalse(ok)
        self.assertIn("fenced", msg)

    def test_yesterdays_entry_does_not_back_a_relay(self):
        import datetime
        y = (datetime.date.today() - datetime.timedelta(days=1)).isoformat()
        root = self._vault(self._entry(3, 3, date=y))
        ok, msg = self.mod.check_qa_log_backing("process-qa", self._relay(3, 3, 3), root=root)
        self.assertFalse(ok)

    def test_a_later_entry_from_another_session_does_not_invalidate_todays_match(self):
        """Build review N10: only the last entry was read, so a second session
        appending to the same log the same day false-blocked the first."""
        root = self._vault(self._entry(3, 3) + self._entry(5, 5))
        ok, msg = self.mod.check_qa_log_backing("process-qa", self._relay(3, 3, 3), root=root)
        self.assertTrue(ok, msg)

    def test_no_verifiable_claims_alone_satisfies_the_scope_check_too(self):
        """Build review N14: the exit phrase cleared the report check and then
        blocked on Missing QA SCOPE."""
        ok, _ = self.mod.check_scope("process-qa", ["no verifiable claims"])
        self.assertTrue(ok)

    def test_other_skills_are_not_checked(self):
        ok, _ = self.mod.check_qa_log_backing("process-build", self._relay(3, 3, 3), root=None)
        self.assertTrue(ok)

    def test_end_to_end_a_forged_relay_blocks_and_a_backed_one_does_not(self):
        """The hook as a subprocess, on an isolated vault tree, with a transcript
        of the real three-entry shape: Skill(process-qa), a tool result, the relay."""
        import json as _json, os as _os, subprocess, sys as _sys, tempfile, pathlib
        from _hooktest import isolate
        for forged, expect_block in ((True, True), (False, False)):
            tmp = pathlib.Path(tempfile.mkdtemp(prefix="qa-e2e-"))
            hook = isolate("process-step-check.py", tmp)
            root = tmp / "vault"
            log = root / "Projects" / "Demo" / "work" / "qa-log.md"
            log.parent.mkdir(parents=True)
            log.write_text("# log\n" + self._entry(2, 2), encoding="utf-8")
            relay = self._relay(2, 5 if forged else 2, 5 if forged else 2)[0]
            lines = [
                _json.dumps({"type": "user", "message": {"role": "user", "content": "go"}}),
                _json.dumps({"type": "assistant", "message": {"content": [
                    {"type": "tool_use", "name": "Skill", "input": {"skill": "process-qa"}}]}}),
                _json.dumps({"type": "user", "message": {"role": "user", "content": [
                    {"type": "tool_result", "content": "Launching skill"}]}}),
                _json.dumps({"type": "assistant", "message": {"content": [{"type": "text", "text": relay}]}}),
            ]
            tp = tmp / "transcript.jsonl"
            tp.write_text("\n".join(lines) + "\n", encoding="utf-8")
            ...
            r = subprocess.run([_sys.executable, str(hook)], input=_json.dumps({"transcript_path": str(tp)}),
                               capture_output=True, text=True, timeout=60, env=env)
            self.assertEqual(r.returncode, 0, r.stderr)
            self.assertEqual('"block"' in r.stdout, expect_block, f"forged={forged}: {r.stdout}")

    def test_the_block_switch_has_both_polarities(self):
        """Mutation check on RELAY_BACKED_BLOCK: on, an unbacked relay is a hard
        block; off, it is a stderr warning and the turn passes."""
        import json as _json, os as _os, subprocess, sys as _sys, tempfile, pathlib, re as _re
        from _hooktest import isolate
        for on, expect_block in ((True, True), (False, False)):
            tmp = pathlib.Path(tempfile.mkdtemp(prefix="qa-switch-"))
            hook = isolate("process-step-check.py", tmp)
            src = hook.read_text(encoding="utf-8")
            self.assertIn("RELAY_BACKED_BLOCK = True", src)
            hook.write_text(src.replace("RELAY_BACKED_BLOCK = True", f"RELAY_BACKED_BLOCK = {on}"), encoding="utf-8")
            relay = self._relay(2, 2, 2, path="Nowhere/work/qa-log.md")[0]
            lines = [
                _json.dumps({"type": "user", "message": {"role": "user", "content": "go"}}),
                _json.dumps({"type": "assistant", "message": {"content": [
            ...
            if not on:
                self.assertIn("QA relay not backed", r.stderr)

    def test_the_relay_is_checked_even_when_the_skill_call_is_outside_the_window(self):
        """Measured on this session's own transcript (2026-09-21): 1.9 MB of tool
        output sat between Skill(process-qa) and the reply, so the 200 KB window
        never saw the invocation and the relay went unchecked (finding 17 in the
        adversarial review). The relay is self-identifying; it is checked from
        the last assistant text whether or not the skill call is visible."""
        import json as _json, os as _os, subprocess, sys as _sys, tempfile, pathlib
        from _hooktest import isolate
        tmp = pathlib.Path(tempfile.mkdtemp(prefix="qa-window-"))
        hook = isolate("process-step-check.py", tmp)
        filler = _json.dumps({"type": "user", "message": {"role": "user", "content": [
        ...
        tp = tmp / "transcript.jsonl"
        tp.write_text("\n".join(lines) + "\n", encoding="utf-8")
        self.assertGreater(tp.stat().st_size, 250_000)
        env = dict(_os.environ, PYTHONIOENCODING="utf-8",
                   GOVERNANCE_LOG_PATH=str(hook.parent / "governance-log.jsonl"),
                   HOOK_ACTIVITY_LOG_PATH=str(hook.parent / "hook-activity.jsonl"))
        r = subprocess.run([_sys.executable, str(hook)], input=_json.dumps({"transcript_path": str(tp)}),
