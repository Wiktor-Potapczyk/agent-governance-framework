...
# Agent types built into Claude Code that no registry lists (silent-failure
# review item 6, 2026-09-21): declaring one of them read as an empty list.
BUILTIN_AGENT_TYPES = frozenset({"explore", "general-purpose", "plan", "web-fetch", "fork", "claude"})
...
def load_known_dispatch_names(fallback, warn_label=None):
    ...
    try:
        ...
        if isinstance(names, list) and names:
            return set(names) | BUILTIN_AGENT_TYPES
    ...
    if warn_label:
        try:
            ...
            pass
    return set(fallback) | BUILTIN_AGENT_TYPES
