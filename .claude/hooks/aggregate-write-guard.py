...
# .claude/hooks/<this file> -> .claude/hooks -> .claude -> vault root. Same
# self-locating idiom as the rest of the converted harness; parents[2] there,
# two dirnames here because this file predates pathlib use in the guards.
_VAULT_ROOT = os.path.dirname(os.path.dirname(_HOOKS_DIR))


def _project_key(vault_root):
    """Claude Code's own project-directory name: the absolute path with the
    drive colon, every separator and every space replaced by '-'.

    The same rule as blueprint_render.compute_project_key. Deliberately
    re-derived here in four lines rather than imported: this is a Gate-1
    PreToolUse guard, and importing .claude/scripts/blueprint_render would
    pull o17_portability_manifest and mirror_user_claude into every tool
    call. memory-context-guard.py carries the same four lines for the same
    reason; if the rule ever changes, both copies and blueprint_render
    change together.
    """
    win = vault_root.replace("/", chr(92))
    return win.replace(":", "-").replace(chr(92), "-").replace(" ", "-")


_DEFAULT_MEMORY_PATH = os.path.join(
    os.path.expanduser("~"), ".claude", "projects",
    _project_key(_VAULT_ROOT), "memory", "MEMORY.md",
)
