...
KNOWN LIMITATION (review finding 3, documented 2026-08-31): the history exemption
is heading-scoped, so content APPENDED AT FILE END after a history-shaped heading
(the live STATE.md ends under "## last_action history") is structurally invisible
to Rule A until a fresh non-history heading follows it. Append-at-end is a common
editing pattern; a contradiction written there will not fire. Working as designed
rather than a defect, because un-exempting history reintroduces the flag-the-audit-
trail failure this docstring names above; recorded so the silence is a known trade,
not a surprise.
...
LEADING_CLOSE = re.compile(
    r"^\s*(?:[-*]\s*)?(?:\*\*|__)?\s*"
    r"(SUPERSEDED|CLOSED|DONE|SHIPPED|COMPLETED?|LANDED|RESOLVED|FOLDED)\b",
    re.IGNORECASE,
)
...
# "e.g."/"i.e." are abbreviations: splitting there separated an OPEN marker
# from the task number it modifies and a real contradiction went undetected
# (calibration ticket 2026-08-07, fixed 2026-08-31).
CLAUSE_BOUNDARY = re.compile(r"(?<!\be\.g\.)(?<!\bi\.e\.)(?<=[.;])\s+",
                             re.IGNORECASE)  # "E.g." must not split either (review 2026-08-31)
...
def find_contradictions(lines: list[str]) -> list[str]:
    ...
    for i, line in enumerate(lines, 1):
        ...
        if leads_with_verdict or line_closed_checkbox:
            # Structural close, clause-scoped since 2026-08-31: the verdict clause
            # closes its own numbers, and those numbers stay closed against any
            # OPEN_MARKER text later in the same line (defect-5 protection: a
            # closed entry quoting open-sounding history about its own subject
            # does not reopen it). What changed: an open claim about a DIFFERENT
            # task in a later clause is no longer absorbed by the verdict; it is
            # scored like ordinary prose (board line 392b, demonstrated by the
            # architect review as a silently-missed genuine contradiction).
            clauses = _clause_split(line)
            line_closed_here: set[str] = set(_task_numbers_loose(clauses[0], known))
            line_opened_here: set[str] = set()
            for n in line_closed_here:
            ...
            for clause in clauses[1:]:
                ...
                for n in _task_numbers_loose(clause, known):
                    if clause_open:
                        if n in line_closed_here:
                            continue
                        open_tasks.setdefault(n, i)
                        line_opened_here.add(n)
                    else:
                        closed_tasks.setdefault(n, i)
                        line_closed_here.add(n)
                        # An open-then-close arc narrated within one line is a
                        # transition, not a contradiction (review finding 2,
                        # 2026-08-31, mirroring Rule B's transition philosophy):
                        # drop only the open registration THIS line created.
                        if n in line_opened_here and open_tasks.get(n) == i:
                            del open_tasks[n]
                            line_opened_here.discard(n)
            continue
