...
def test_regression_task_222_range_phrase_survives_a_later_unrelated_marker():
    ...
    a DIFFERENT, later clause in the same paragraph (a queued Wiktor decision), not
    task 222's own status."""
    out = find(
        "**SHIPPED 2026-08-06 late evening: Harness Utilization Analysis** "
        "(task_plan tasks 222 to 227). Wiktor's ask after the context-engineering "
        ...
        "Wiktor is owner-gated but not externally blocked.\n"
    )
    assert out == []
...
def test_regression_task_232_origin_citation_does_not_reopen_it():
    ...
    out = find(
        ...
        "surfaced a session-critical fact: Wiktor believes the config gate is set, "
...
# --- Calibration tickets closed 2026-08-31 (board lines 392 + 435) ------------


def test_folded_leading_verdict_governs_its_line():
    # 435 candidate rule: FOLDED joins the LEADING_CLOSE verdict list, so a line
    # that leads with it cannot reopen its own subject via quoted body text.
    text = (
        "- [x] task 640 done, verified.\n"
        "FOLDED (2026-08-07): task 640 STILL OPEN in the old wording, kept for audit.\n"
    )
    assert find(text) == []


def test_eg_does_not_split_marker_from_its_number():
    # 392(a): "e.g." must not be a clause boundary; pre-fix the OPEN marker and
    # its task number land in different clauses and a real contradiction is missed.
    text = (
        "- [x] task 641 SHIPPED with the review pass.\n"
        "The follow-up is STILL OPEN, e.g. task 641 needs the sign-off.\n"
    )
    assert len(find(text)) == 1


def test_ie_does_not_split_marker_from_its_number():
    text = (
        "- [x] task 643 SHIPPED.\n"
        "One item is AWAITING review, i.e. task 643 has to be re-checked.\n"
    )
    assert len(find(text)) == 1


def test_open_claim_about_another_task_inside_closed_bullet_fires():
    # 392(b): the closed side is clause-scoped; a genuine open claim about a
    # DIFFERENT task inside a "- [x]" bullet is not silently absorbed.
    text = (
        "- [x] task 650 SHIPPED. This unblocks task 651, which is AWAITING sign-off.\n"
        "- [x] task 651 CLOSED with QA PASS.\n"
    )
    findings = find(text)
    assert len(findings) == 1 and "651" in findings[0]


def test_closed_bullets_own_body_cannot_reopen_it():
    # Defect-5 protection must survive 392(b): the checkbox still wins outright
    # for the numbers its own line closes, even against a later open clause.
    text = (
        "- [x] task 222 DONE with QA PASS. But task 222 is AWAITING Wiktor in the preserved text.\n"
        "task 222 shipped earlier, CLOSED.\n"
    )
    assert find(text) == []


def test_capitalized_eg_does_not_split_either():
    # Review finding 1 (2026-08-31): sentence-initial "E.g." must behave like
    # "e.g."; without IGNORECASE the lookbehind fails and the ticketed miss returns.
    text = (
        "- [x] task 642 SHIPPED.\n"
        "The follow-up is STILL OPEN, E.g. task 642 needs the sign-off.\n"
    )
    assert len(find(text)) == 1


def test_open_then_close_in_one_closed_line_is_a_transition_not_a_contradiction():
    # Review finding 2 (2026-08-31): a closed line narrating an arc for another
    # task ("was AWAITING... now CLOSED") must not fire a same-line
    # self-contradiction; the later close supersedes this line's own open claim.
    text = (
        "- [x] task 700 done. Task 701 is AWAITING review. Task 701 CLOSED per follow-up.\n"
    )
    assert find(text) == []


def test_earlier_lines_open_claim_survives_a_later_lines_close_arc():
    # Guard on the finding-2 fix: dropping this line's own open registration must
    # not erase an open claim recorded by a DIFFERENT line.
    text = (
        "task 702 is AWAITING review.\n"
        "- [x] task 703 done. Task 702 CLOSED per follow-up.\n"
    )
    findings = find(text)
    assert len(findings) == 1 and "702" in findings[0]
