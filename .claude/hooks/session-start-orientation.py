...
3. Fallback: 'Agent-Governance-Research' (vault stewardship hub)
...
FALLBACK_PROJECT = "Agent-Governance-Research"
...
def get_cost_line():
    ...
    try:
        ...
        if not os.path.isfile(script):
            return ""
        # The interpreter that is already running this hook, not a typed
        # path: a hard-coded C:/Program Files/Python314/python.exe made this
        # call silently degrade to "" behind the bare except on any machine
        # that installs Python elsewhere.
        interpreter = sys.executable or "python"
        result = subprocess.run(
            [interpreter, script, "--hours", "24", "--json"],
