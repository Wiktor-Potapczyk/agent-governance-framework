...
Spec: Projects/Agent-Governance-Research/work/2026-08-17-hermes-p2-memory-governance-plan.md
...
# .claude/hooks/<this file> -> .claude/hooks -> .claude -> vault root.
...
def _project_key(vault_root):
    ...
    Same rule as blueprint_render.compute_project_key, re-derived in four
    lines rather than imported so that a PreToolUse guard does not pull the
    blueprint module chain into every tool call. aggregate-write-guard.py
    carries the same four lines; the two change together.
    """
...
_DEFAULT_MEMORY_DIR = os.path.join(
    ...
    _project_key(_VAULT_ROOT), "memory",
)
