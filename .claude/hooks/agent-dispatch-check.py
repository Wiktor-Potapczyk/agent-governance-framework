...
def _emit_dispatch(session_id, agent_type, must_dispatch, exempted, warn, outcome):
    ...
    behind 611 of 931 unmatched skill-side join items on the 2026-08-30
    evidence. Partition: entries that are registry agent names (local +
    plugin, via load_registry_agents) go to the NEW agent_context field;
    the remainder stays in skill_context. Agent membership wins when a
    name is in both sets (measured overlap: n8n-reviewer). Fail-open: any
    partition error falls back to the old unpartitioned shape -- a
    dispatch must never break because logging broke. Historical sink lines
    are never rewritten; the record-shape change is owner-acknowledged
    (O13 ratification ask 3, 2026-09-01).
    """
