...
class EmissionGateTests(unittest.TestCase):
    """R17 tune (owner 'ok go' 2026-09-06): only the worst-case shape emits.

    Sampling found ~70 percent of emitted events were routine turns already
    self-classified adequate; the gate keeps the event stream to the shape
    the detector's own worst-case definition names (zero citations AND zero
    file writes)."""

    def _transcript(self, td, n_agents, n_writes, final_text):
        tp = Path(td) / "session-gate.jsonl"
        blocks = []
        for i in range(n_agents):
            blocks.append({"type": "tool_use", "name": "Agent",
                           "input": {"subagent_type": "t%d" % i, "description": "x"}})
        for i in range(n_writes):
            blocks.append({"type": "tool_use", "name": "Write",
                           "input": {"file_path": "f%d.md" % i}})
        entries = [
            {"type": "user", "message": {"content": "do thing"}},
    ...
    def _run_collecting(self, tp):
        records = []

        def _rec(event, hook, session, extra=None, environment=None):
            records.append({"event": event, "hook": hook, "extra": extra or {}})

        import _event_emit
        with mock.patch.object(_event_emit, "emit_event", _rec):
            with mock.patch.object(sys, "stdin",
                                   __import__("io").StringIO(
                                       json.dumps({"transcript_path": str(tp)}))):
                dzc.main()
    ...
    def test_low_severity_does_not_emit(self):
        with tempfile.TemporaryDirectory() as td:
            tp = self._transcript(td, n_agents=1, n_writes=1, final_text="Done.")
            self.assertEqual(self._run_collecting(tp), [])

    def test_medium_severity_does_not_emit(self):
        with tempfile.TemporaryDirectory() as td:
            tp = self._transcript(td, n_agents=3, n_writes=1, final_text="Done.")
            self.assertEqual(self._run_collecting(tp), [])

    def test_worst_case_still_emits(self):
        with tempfile.TemporaryDirectory() as td:
            tp = self._transcript(td, n_agents=1, n_writes=0, final_text="Done.")
            records = self._run_collecting(tp)
            self.assertEqual(len(records), 1)
            self.assertEqual(records[0]["event"], "dark-zone")
            self.assertEqual(records[0]["extra"]["severity"], "high")
