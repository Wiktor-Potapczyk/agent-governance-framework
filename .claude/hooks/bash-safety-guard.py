...
_IRREVERSIBLE_FALLBACK_SNAPSHOT = [
    ...
    # P5 — prod deploy. The degraded fallback carries NO project exemption: the
    # exempt build-oracle project is named in _irreversible_surface_local.json,
    # which this snapshot deliberately does not read, so the fallback denies a
    # compose up the canonical surface would have exempted. Stricter is the
    # right direction for a path that only runs when Gate-1 is already degraded.
...
# Wiktor ruling 2026-08-13: writes to hosts we own warn instead of denying. The
...
BLOCKED_PATTERNS = [
    ...
    # Force-push naming main/master, and force-push that names no branch at all.
    # Wiktor 2026-08-25: force-push to a WORKING branch warns (see WARN_PATTERNS below);
    # main is what other clones and CI are pinned to, so rewriting it stays denied, and a
    # refspec-less force-push could BE main, so it is denied on the same ground.
    # These sit ABOVE the generic rows so they claim the match and keep their own
    # description; the generic rows below are the ones the warn filter lifts out.
    (r'\bgit\s+(?:(?:-C\s+\S+|-c\s+\S+|--[A-Za-z][\w-]*(?:=\S+)?|-[A-Za-z])\s+)*push\b(?=[^;|&]*(?:--force|(?<![\w-])-f(?![\w-])))[^;|&]*(?<![\w.-])ma(?:in|ster)(?![\w.-])',
     "git force-push to main"),
    (r'\bgit\s+(?:(?:-C\s+\S+|-c\s+\S+|--[A-Za-z][\w-]*(?:=\S+)?|-[A-Za-z])\s+)*push\b(?=[^;|&]*(?:--force|(?<![\w-])-f(?![\w-])))(?![^;|&]*:[\w./-]+)(?![^;|&]*\s(?!-)[\w./-]+\s+(?!-)[\w./-]+)',
     "git force-push with no named branch"),
...
# human gate is Wiktor re-running it himself via the `!`-prefix bypass. In
...
# Wiktor 2026-08-25, the same reasoning he applied to normal push on 2026-08-05: the
# !-prefix round trip moved no decision to a human, because the command he pasted was
# the one the agent had just composed and handed him. Force-push to a working branch
# joins the warn tier on that ground. It is NOT the same act as force-push to main,
# which the two rows above keep denied. Still logged to governance-log.jsonl as `warn`,
# so the audit trail keeps every history rewrite.
_WARN_PUSH_DESCS = {
    _NORMAL_PUSH_DESC,
    "git force-push",
    "git force-push (-f)",
}
WARN_PATTERNS = [(p, d) for (p, d) in BLOCKED_PATTERNS if d in _WARN_PUSH_DESCS]
BLOCKED_PATTERNS = [(p, d) for (p, d) in BLOCKED_PATTERNS if d not in _WARN_PUSH_DESCS]
...
def main():
    ...
    # Valid JSON of the wrong shape is still unusable input. Applied 2026-09-11
    # under an explicit owner OK, since R13 keeps edits to this file owner-gated;
    # it was the one registered hook held back from the house-wide pass. Returning
    # here matches the existing unparseable-stdin branch two lines up, so the
    # Gate-1 deny path is unchanged: a payload with no readable command was never
    # going to produce a deny either way.
    ...
    # Warn-only surface (Wiktor ruling 2026-08-05): runs AFTER the deny loop, so a
    ...
    if curl_deny:
        # Wiktor ruling 2026-08-13: a write aimed ONLY at infrastructure we own
