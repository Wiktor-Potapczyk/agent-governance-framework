 ...
 * Source draft: Projects/Agent-Governance-Research/work/2026-06-11-procedure-layer-migration-plan.md Part C §process-research.
...
// ---------------------------------------------------------------------------
// >>> LIVE_GATE_HELPER_START  (pure, side-effect-free, unit-testable in isolation)
function evaluateLiveCitationGate(input) {
  const liveRequired = !!(input && input.liveRequired)
  const liveCitationCount = Number(input && input.liveCitationCount) || 0
  // Conservative: fail ONLY when scope explicitly required live verification AND zero live citations exist.
  // Not-required => always pass (strict no-op). Required + >=1 live citation => pass.
  if (!liveRequired) {
    return { liveGatePass: true, reason: 'live verification not required by scope (no-op).' }
  }
  if (liveCitationCount > 0) {
    return { liveGatePass: true, reason: `live verification required and satisfied: ${liveCitationCount} live citation(s) found.` }
  }
  return { liveGatePass: false, reason: 'scope required live web verification of existence/frontier claims but the report carries ZERO live-verified citations, likely a training-only reconstruction.' }
}
// <<< LIVE_GATE_HELPER_END
...
const SCOPE_SCHEMA = {
  type: 'object',
  required: ['scope_block', 'output_path', 'coverage', 'ralph_loop_indicated', 'live_verification_required', 'rationale'],
  properties: {
    ...
    live_verification_required: { type: 'boolean', description: 'true ONLY if the scope text explicitly demands live/current verification of EXISTENCE, FRONTIER, or CREDIBILITY claims (e.g. "does X exist", "latest/current state of", "verify this is real", "find the actual URL/source", "as of <recent date>"). false for established-literature / conceptual / training-KB questions (definitions, well-known papers, lineage explanations). This is a SCOPE-LEVEL gate, not a per-sentence judgment — when in doubt, false (conservative: do not mandate live for conceptual asks).' },
...
const QUALITY_SCHEMA = {
  type: 'object',
  required: ['report_file_exists', 'all_questions_addressed', 'sources_cited', 'live_verification_required', 'live_citation_count', 'pass', 'evidence'],
  properties: {
    ...
    live_verification_required: { type: 'boolean', description: 'echo of the scope flag: true if the scope demanded live verification of existence/frontier/credibility claims.' },
    live_citation_count: { type: 'integer', description: 'count of DISTINCT live-verified source citations on existence/frontier/credibility claims in the report (a citation backed by a resolvable URL you confirmed via WebFetch/WebSearch, NOT a training-memory reference). 0 if none.' },
    pass: { type: 'boolean', description: 'true only if report_file_exists AND all_questions_addressed AND sources_cited are all true; live_citation_count is evaluated separately in code.' },
...
//   { project:      string          (required) — e.g. "Agent-Governance-Research"
...
3. Classify the routing flags from the ACTUAL scope:
   ...
   - live_verification_required: true ONLY if the scope text EXPLICITLY demands live/current verification of EXISTENCE, FRONTIER, or CREDIBILITY claims ("does X exist", "latest/current state of", "verify this is real", "find the actual URL/source", "as of <recent date>"). Default FALSE for established-literature / conceptual / training-KB questions (definitions, well-known papers, lineage explanations). This is a SCOPE-LEVEL gate, not a per-sentence judgment — when in doubt, FALSE (conservative: do not mandate live for conceptual asks).
...
// PREVENTION layer: when the scope flagged live verification as required, live
// WebSearch/WebFetch becomes NON-OPTIONAL for existence/frontier/credibility claims.
// When not required, the mandate is the empty string — strict no-op so normal runs
// are unaffected (no false-positiving on established-literature/training-KB content).
const liveMandate = scope.live_verification_required
  ? `
LIVE-VERIFICATION MANDATE (scope flagged live_verification_required=true):
For every EXISTENCE / FRONTIER / CREDIBILITY claim (does X exist, latest/current state, is this real, who published it), you MUST run live WebSearch and/or WebFetch and cite a resolvable source URL + access date for that claim. A claim of this class returned WITHOUT a live source MUST be labeled "UNVERIFIED" in your findings. Training-memory reconstruction is NOT acceptable for these claims. Established-literature/conceptual claims (definitions, well-known papers) do not require live citation.`
  : ''
...
4. LIVE-CITATION CHECK. This scope's live_verification_required = ${!!scope.live_verification_required}. Echo it into live_verification_required.
   - If TRUE: run live WebSearch/WebFetch on the report's EXISTENCE/FRONTIER/CREDIBILITY claims and set live_citation_count to the NUMBER of such claims backed by a confirmed resolvable LIVE source (a URL you actually resolved — NOT a training-memory reference). A report of this class that reconstructs frontier claims purely from training memory will have live_citation_count = 0.
   - If FALSE: set live_citation_count = 0 and note in evidence that the live check was skipped (it will not affect the derived pass — established-literature/conceptual reports are unaffected).
...
// ENFORCEMENT layer: derive the conservative live-citation gate from the AUTHORITATIVE
// scope flag (not the gate agent's echo — an agent that drops the echo must not be able
// to silently disable the gate). Fails ONLY when live was required AND zero live citations.
const liveGate = evaluateLiveCitationGate({
  liveRequired: !!(scope && scope.live_verification_required),
  liveCitationCount: quality ? quality.live_citation_count : 0,
})
