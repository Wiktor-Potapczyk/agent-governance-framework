...
def main():
    ...
    def _log(decision, detail=None):
        ...
        try:
            ...
            log_fire("agent-registry-check", decision=decision, detail=detail,
    ...
    if agent_type not in GENERIC_TYPES:
        # Logged rather than dropped: the generic-versus-specialist dispatch
        # ratio is the measurement this filter produces, and no other sink has it.
    ...
    if not agents:
        _log("skip", "registry-unavailable")
        return
    ...
    if specialists:
        ...
        # Emit first, log second. Nothing between the verdict and the stdout
        # payload may be able to swallow it (idiom from routing-table-validation).
        print(json.dumps(response))
        _log("nudge", suggestions)
    else:
