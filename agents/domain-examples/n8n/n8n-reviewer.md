...
- Visual guidelines (layout, connector crossings, sticky placement, execution-view rendering) cannot be graded from JSON, and this agent has no browser tools (its tools list is Read, Write, Edit, Grep, Glob, Bash). Report those items as NOT CHECKED with the workflow URL and what to look at; the orchestrating session can open the operator's browser for a real look. Scoring a visual section from `position` coordinates is a fabricated finding. See `skills/domain-examples/n8n/n8n-review/SKILL.md`, section 3 (Visual Chain).
